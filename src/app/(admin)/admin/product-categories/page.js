...
export default function ProductCategoriesPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editing) {
        await updateProductCategory(editing.id, form, toast);
      } else {
        await createProductCategory(form, toast);
      }
      setModalOpen(false);
      fetchCategories();
    } catch (e) {
  ...
  const handleDelete = async (id) => {
    await deleteProductCategory(id, toast);
    fetchCategories();
  };
          ...
          <div className="relative bg-white rounded-lg p-6 max-w-lg w-full mx-4 shadow-lg">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-semibold text-gray-900">
                {editing ? "Edit Product Category" : "Create Product Category"}
              </h3>
